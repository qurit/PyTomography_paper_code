...
# Change this to where you'd like to save the data
savepath = '/disk1/pytomography_paper_results/simind'

organ_specifications_path = os.path.join(datapath, 'organ_specifications.csv')
organ_concentrations_path = os.path.join(datapath, 'organ_concentrations.csv')
...
CPSperMBq = 11.7284 #calibration factor obtained from SIMIND output
            ...
            for projection_time in projection_times:
                for prior_type, prior_name in zip(prior_types, prior_names):
                    if recon_type=='masked':
                        ...
                        n_iters = 120
                    print('next')
                    save_path = os.path.join(datapath, f'{organ_concentrations_path.split("/")[-1][:-4]}_{organ_concentrations_index}_recon{recon_type}_{prior_name}_scat{scatter_type}_dT{projection_time}_masks{organ_segmentation_type}_niters{n_iters}')
